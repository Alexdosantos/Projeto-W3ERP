...
export const ProdutosDetails = () => {
  ...
  const { data: clienteEmBaixa } = useQuery<Root>(["clienteEmBaixa", id], () =>
    GetUser.clienteEmBaixa(id)
  );
  console.log(clienteEmBaixa);
  
  const { data: clienteEmAlta } = useQuery<Root>(["clienteEmAlta", id], () =>
  ...
  return (
    <>
      <Layout>
        ...
        <S.DivTable>
          <TabelaProducts nav={""} id="ID" dados={clienteEmBaixa}  />
          <TabelaProducts nav={""} id="ID" dados={clienteEmAlta}  />
        </S.DivTable>
      </Layout>
    </>
  );
